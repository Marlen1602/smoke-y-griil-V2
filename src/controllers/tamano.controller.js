...
export const getTamanos = async (req, res) => {
  try {
    const [tamanos] = await sequelize.query("SELECT * FROM tamanoproducto ORDER BY ID_Tamano DESC");
    res.json(tamanos);
  } catch (error) {
...
export const getTamanoById = async (req, res) => {
  const { id } = req.params;
  try {
    const [tamano] = await sequelize.query("SELECT * FROM tamanoproducto WHERE ID_Tamano = ?", {
      replacements: [id],
    });
    if (tamano.length === 0) {
...
export const createTamano = async (req, res) => {
  ...
  try {
    const [result] = await sequelize.query(
      "INSERT INTO tamanoproducto (ID_Producto, Nombre, Precio) VALUES (?, ?, ?)",
      { replacements: [ID_Producto, Nombre, Precio] }
    );
    res.json({ message: "Tamaño creado correctamente", ID_Tamano: result.insertId });
  } catch (error) {
...
export const updateTamano = async (req, res) => {
  ...
  try {
    await sequelize.query("UPDATE tamanoproducto SET Nombre = ?, Precio = ? WHERE ID_Tamano = ?", {
...
export const deleteTamano = async (req, res) => {
  const { id } = req.params;
  try {
    await sequelize.query("DELETE FROM tamanoproducto WHERE ID_Tamano = ?", {
      replacements: [id],
    });
