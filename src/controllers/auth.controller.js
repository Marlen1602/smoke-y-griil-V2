...
export const register= async (req, res) => {
   ...
   try {
    ...
    if (error.code === 11000) {
        const duplicatedField = Object.keys(error.keyValue)[0];  // El campo duplicado
...
export const login= async (req, res) => {
 ...
 export const profile=async (req,res)=>{
    const userfoud= await User.findById(req.user.id)
    if(!userfoud) return res.status(400).json({message:"User not found"});
    return res.json({
        id:userfoud._id,
        username:userfoud.username,
        nombre:userfoud.nombre,
        apellidos:userfoud.apellidos,
        email:userfoud.email,
        createAt:userfoud.createdAt,
